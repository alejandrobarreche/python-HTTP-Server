...
def realizar_solicitud(
    ...
    try:
        ...
        try:
            ...
            contenido = {
                "texto": respuesta.text[:100] + "..."
                if len(respuesta.text) > 100
                else respuesta.text
            }
        ...
        tiempo_total = time.time() - inicio
        logger.warning(
            f"Solicitud {identificador} agotó el tiempo de espera ({timeout}s)"
        )
...
def ejecutar_prueba_concurrente(
        url_base: str, num_solicitudes: int, concurrencia: int, tipo_prueba: str
) -> List[Dict]:
    """
    ...
    Args:
        ...
        tipo_prueba: Tipo de prueba a realizar (get, sleep, post o mixed).
    ...
    logger.info(
        f"Iniciando prueba con {num_solicitudes} solicitudes, {concurrencia} concurrentes"
    )
        ...
        for i, future in enumerate(concurrent.futures.as_completed(futures)):
            try:
                ...
                if (i + 1) % 10 == 0 or i == 0 or i == num_solicitudes - 1:
                    logger.info(
                        f"Completadas {i + 1}/{num_solicitudes} solicitudes"
                    )
            except Exception as e:
    ...
    logger.info(
        f"Éxito: {solicitudes_exitosas}/{num_solicitudes} "
        f"({solicitudes_exitosas/num_solicitudes*100:.1f}%)"
    )
    if tiempos:
        ...
        maximo = max(tiempos)
        logger.info(
            f"Tiempo promedio: {promedio:.3f}s, Min: {minimo:.3f}s, Max: {maximo:.3f}s"
        )
...
def guardar_resultados(
        resultados: List[Dict], analisis: Dict, archivo: str
):
    """
    ...
    Args:
        ...
        archivo: Nombre base del archivo donde guardar los resultados.
    """
    datos = {
        "timestamp": time.time(),
    ...
    archivo = f"{archivo}_{timestamp_str}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Cliente de prueba para servidor HTTP concurrente"
    )
    parser.add_argument(
        "--host",
        default="localhost",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Número total de solicitudes (default: 100)",
    )
    parser.add_argument(
        "--concurrencia",
        type=int,
        default=10,
        help="Nivel de concurrencia (default: 10)",
    )
    parser.add_argument(
        "--tipo",
        choices=["get", "sleep", "post", "mixed"],
        default="mixed",
        help="Tipo de prueba a realizar (default: mixed)",
    )
    parser.add_argument(
        "--output",
        default="./data/resultados_prueba",
        help="Archivo base de salida para resultados (default: ./data/resultados_prueba)",
    )
    ...
    for codigo, cantidad in sorted(analisis["codigos_respuesta"].items()):
        print(
            f"  {codigo}: {cantidad} ({cantidad/analisis['total_solicitudes']*100:.1f}%)"
        )
